...
with header:
    ...
    st.image(back_img)
    st.markdown('Pokemons são criaturas fictícias do universo Pokemon. Eles existem em todas as formas e tamanhos, e todos têm habilidades diferentes, como ataques de fogo, ataques elétricos e ataques de gelo. Os jogadores coletam e treinam essas criaturas para usá-las em batalhas contra outros treinadores. O objetivo é coletar o maior número possível de Pokémons e torná-los os mais poderosos possível.')

with dataset:
    st.header('Conheça seus pokemons:')
    poke_len = st.slider('Determine quantos pokemons você vai listar:', 1, 5, (1, N_POK))
...
with explorer:  
    ...
    sel_generation = st.selectbox(
        'Qual a geração para o plot?',
        (1, 2, 3, 4, 5, 6)
    )
