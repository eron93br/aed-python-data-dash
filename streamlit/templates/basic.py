...
with stats_container:
	...
	s_station = st.selectbox('Que estação inicial vc gostaria de ver?', start_station_list, key='start_station')
	...
	multi_select = st.multiselect('QUAL ESTAÇÃO INICIAL?',start_station_list, key='start_station', default=['Harborside','Marin Light Rail'])
	slider_input = st.slider('Quão longa deve ser essa viagem?', int(data['tripduration'].min()/3600), int(data['tripduration'].max()/3600), 25)
